...
class Editproject extends React.Component {
    ...
    componentDidMount(){
        ...
        .then(response => {
          ...
          this.setState({
            ...
            logfile:response.data.logfile,
            connectiontimout:response.data.connectiontimout,
            sendtimeout:response.data.sendtimeout,
            receivetimeout:response.data.receivetimeout,
          });
        })
    ...
    onSubmithandle(){
       ...
       if(typeof(this.state.logfile) == 'object'){
        console.log('yes')
        let apiUrl;
        apiUrl= CONFIG.insertproject+this.state.id;
        ...
        axios.put(apiUrl, formData)
            .then(response => {
            console.log(response)
            if(response.data.message== "Project data Update successfully!"){
                this.setState({
          ...
          let data ={ "projectname": this.state.projectname, "description": this.state.description, "targeturl": this.state.targeturl, "logfile": this.state.logfile, "connectiontimout": this.state.connectiontimout, "sendtimeout": this.state.sendtimeout, "receivetimeout": this.state.receivetimeout}
  ...
  render() {
    return (
        ...
        </svg><span style={{marginLeft:'1%'}}>Edit project </span>  
        </p></div>
