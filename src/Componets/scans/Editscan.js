...
class Editscan extends React.Component {
    ...
    let apiUrl;
    apiUrl= CONFIG.get_scan_by_id+this.state.id;
    axios.get(apiUrl)
    .then(response => {
    console.log(response)
    this.setState({
    projectname:response.data.projectname,
    scandatetime:response.data.scandatetime,
    ...
    onSubmithandle(){
        ...
        axios.put(apiUrl, data)
            .then(response => {
            console.log(response)
            if(response.data.message=="Scan was update successfully!"){
                this.setState({
                    message:'Scan was update successfully!',
                    messageshow:true,
...
this.props.history.push("/"); 
  ...
  render() {
    return (
        ...
        </svg><span style={{marginLeft:'1%'}}>Edit scans </span>  
        </p></div>
  ...
  <p> Test mode:</p>
    ...
    <div class='select project' style={{marginTop:'3%'}} >
            ...
            <div class="container" >
               ...
               <button class="btn btn-success s" onClick={this.onSubmithandle} type='button' >Update</button><button class="btn btn-success s ds" type='button' onClick={this.scacaincile} >Cancel</button>
               </div>
                </div>
                </div>
